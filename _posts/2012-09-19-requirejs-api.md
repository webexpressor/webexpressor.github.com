...
# 1. 用法
## 1.1 加载JavaScript文件
RequireJs加载脚本和传统的&lt;script>标签不同，它可以运行和优化的很好，它建议使用模块化的代码，如尽量使用Models ID，而不是URL。

RequireJs相对于baseUrl来加载代码的，baseUrl通常设置为和data-main属性值中脚本的目录相同，并作为加载本页中脚本的根目录。RequireJs就是根据data-main属性来开始加载脚本的。例如：

{% highlight javascript %}
<--baseUrl会被设置为scripts目录-->
<script data-main="scripts/main.js" src="scripts/require.js"></script>
{% endhighlight %}
baseUrl也可以通过RequireJs config进行设置，如果没有明确的设置，并且data-main也没有被使用，那么它就会使用加载RequireJs的html的目录。

RequireJs默认认为所以的依赖都是脚本，所以module IDs不需要写“.js”后缀，RequireJs在将module ID转化成路径时会自动加上它。在路径配置（paths config）中，与传统的&lt;script>标签相比，你可以使用很简短的字符串。

有时候你想更直接的去引用一个脚本，而不是根据"baseUrl+path"的规则来，这时如果你的module ID符合下面的规则，该ID不会使用"baseUrl+path"规则，而是被对待为相对于文档的URL。

* ▪ 以".js"结尾
* ▪ 以"/"开始
* ▪ 包括URL协议，如"http:"或"https:"

一般来说，最好是用baseUrl和path来设置module IDs，这样做的话，在优化构建（optimization builds）时就可以更灵活的重命名和配置路径来指向其脚本位置。

同样的，为了避免大量的配置，最好要避免深层次的目录结构，你可以将所有脚本放到baseUrl目录下，或者可以将你的脚本和第三方的放在不同的文件夹下，例如下面这样：
{% highlight javascript %}
▪ www/
	▪ index.html
	▪ js/
...
{% endhighlight	%}
在该例中，像第三方类库jquery，在文件名中并没有使用版本号。如果你想跟踪版本，强烈建议将版本信息存储在一个单独的文件中，或者可以使用[volo]()这个工具，它会在package.json文件中加盖版本信息，但是磁盘上的文件仍然以"jquery.js"命名。这样可以使用简短的配置，而不是将版本信息也包括进去。例如：配置"jquery"就代表"jquery-1.7.2"。

理想情况下，通过define()定义的脚本会被当做为模块(module)。但是，获取你需要加载没有通过define()定义的全局脚本，这时你可以使用shim config，可以正当地描述依赖关系。

如果你没有描述依赖关系，在加载的时候就可能会出错，因为RequireJs为了加快速度会采用异步和无序的方式来加载。

## 1.2 模块的定义

模块与传统的脚本文件不同，它是良好作用域的对象（well-scoped）,可以避免污染全局命名空间。它可以明确地列出其依赖关系，并且处理依赖时不会涉及到global对象，而是这些依赖会作为参数传递给定义模块的函数。RequireJs中的模块是[Module Pattern]()的扩展，优势是引用其它模块时不需要使用global对象。

RequireJs中的模块会尽可能快地被加载，甚至是无序的方式，但是使用时是按照正确的依赖顺序。因为global对象没有被使用，所以在同一页面中可以为一个模块加载不同的版本。

(如果你熟悉或者使用过CommonJs Modules，请参见[CommonJS Notes]()，会说明怎样将CommonJS Module映射为RequireJs Module)。

磁盘上的每个文件只可以定义一个模块，但是通过优化工具可以将多个模块组织在一起。
...
该函数不会被调用，直到依赖的模块加载好之后，并且通过"cart"和"inventory"参数来接收模块。

模块被定义成全局的是不建议这么做的，因为有可能在同一页面针对同一模块会加载不同的版本（参见[高级用法]()）。并且，函数参数的顺序要与依赖数组对应。
...
{% endhighlight%}
模块名称通常是由优化工具生成的，你可以明确指定模块名，但是这样做使得模块变得移植性差--如果你将该文件移到其它的文件夹，你得修改模块名。通常不要自己命名模块，让优化工具去处理。优化工具需要加上模块名，所以可以将各个模块放大一个文件中去，这样也是为了浏览器能更快的加载。

### 1.2.7 模块其它知识
*一个模块一个文件：*每个JavaScript文件只定义一个模块，这样会给模块赋有name-to-path的查询算法。通过优化工具大量的模块会被合并到一个优化文件中，但是仅可以通过优化工具来合并。

*define()中的相对模块名：*例如require('./relative/name')可能会在define()函数体内被调用，这样就要求依赖"require"，一边relative name可以被正确地解析。
{% highlight javascript %}
...
{% endhighlight%}
注意：这只会在通过require(["module/name"])的异步加载的方式下工作，如果使用相对路径，例如：'./module/name'，这只会在define内容执行。
...
下面是一个例子，调用JSONP API端点。在这个例子中，JSONP的回调参数就为"callback"，所以"callback=define"告诉API使用define()将JSON格式的返回对象包装起来。
{% highlight javascript %}
...
{% endhighlight%}
你也可以用全局变量require引用配置对象在require.js未被加载之前，配置信息就会被自动应用。
{% highlight javascript %}
<script>
    var require = {
...
<script src="scripts/require.js"></script>
{% endhighlight%}
*注意：*最好使用var require = {}，不要使用window.require = {}，它在IE中工作不正常。

支持的配置选项：

baseUrl:所有模块查询基于的根路径。所以在上面的例子中，"my/module"的script tag中的src值就为"/another/path/my/module.js"。baseUrl不会加载.js文件，所以a.js和b.js会从相对于该脚本当前目录直接加载。

如果在配置中没有明确地设置baseUrl，它的默认值就为加载require.js的html的目录所在的位置。如果指定data-main属性值，则该路径就为baseUrl。

在加载require.js页面中的baseUrl的URL可以是不同域的。RequireJs的脚本加载可以是跨域的。唯一的限制是加载​​文本的text!插件：至少在开发环境中，他们的路径应该和当前页面在同一域下。在使用优化工具之后，它就会在text!插件资源里，你在其它的域就可以引用text!插件资源来使用该资源了。

paths:是为没有直接在baseUrl中找到模块名而作路径映射。路径的设置时相对于baseUrl，除非使用"/"或者URL协议(如"http:")，用上面的例子说明："some/module"的script tag的src为"/another/path/some/v1.0/module.js"，被用作模块名的路径中不应该包括'.js'，因为路径是针对文件夹的。将模块名映射为路径时会自动加上'.js'。
...
{% endhighlight%}
在IE中如果想要404检测，可以paths fallbacks或errbacks，这时需要exports一个字符串值，以便用来检测脚本被正确加载：
{% highlight javascript%}
requirejs.config({
    shim: {
        'jquery.colorize': {
...
这是在基于上下文的多版本支持是不肯能的。另外paths配置只是为模块ID创建根路径，而不是将一个模块ID映射到另一个模块。
